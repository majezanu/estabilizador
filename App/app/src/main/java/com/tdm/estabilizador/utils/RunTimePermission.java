package com.tdm.estabilizador.utils;
...
public class RunTimePermission extends Activity {
    ...
    public void requestPermission(String[] permissions, RunTimePermissionListener runTimePermissionListener)
    {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
        {
            ...
            for (PermissionBean bean: arrayListPermission)
            {
                arrayPermissions[arrayListPermission.indexOf(bean)] = bean.permission;
            }
    ...
    public void setAlertMessage()
    {
        ...
        {
            Log.v("log_tag: %s", "either activity finish or message length is 0");
        }
    }
    ...
    {
        for (PermissionBean bean:arrayListPermission) {
            if(bean.permission.equals(permissions)){
                if (grantResults == 0)
                {
                    bean.isAccept = true;
                }
